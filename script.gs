...
function doGet(e) {
  try {
    ...
    // Свежий пропуск портала на 30 дней (страница на sb3.acons.space просит раз в сутки)
    ...
    if (action === 'checkPassword') {
      // Пропуск портала (2026-09-15): роль «администратор» = вход администратора
      if (p.pp) {
...
function doPost(e) {
  try {
    ...
    // Пропуск портала acons.space (2026-09-15): пришло поле `pp` — проверяем его ДО всего остального.
    // Все POST-действия — запись, поэтому нужен годный пропуск с ролью «администратор»;
    // иначе (подпись/срок/роль снята/роль «просмотр») — bad_pass, ничего не пишем.
    // Годный пропуск: права администратора и автор = ФИО из пропуска (имя с клиента не используется).
    // Без `pp` — всё ровно как раньше (пароль администратора / запись клеток без пароля).
    var portal = null;
    if (body.pp) {
...
// ───────────── Вход через портал acons.space (2026-09-15, acons-server/TZ.md §15 шаг 1) ─────────────
// Страница на sb3.acons.space получает от портала подписанный пропуск ?p= и шлёт его в бэк
// параметром/полем `pp` (`t=` в СБ3 — метка против кэша, `p` не используем). Копия схемы Отделки:
//   1. checkPortalPass_ — подпись HMAC-SHA256 общим секретом PORTAL_SECRET, код админки 'sb3', срок, роль;
...
// Подрядчики по ?contractor= и чтение без входа не меняются (перевод подрядчиков — §14, позже).
...
function portalWho_(pp) {
  ...
  if (live && !live.role) return null;   // отключён, срок вышел или роль в СБ3 снята
...
/**
 * Текущая роль человека в СБ3 по данным сервера портала acons.space, кэш 5 минут.
 * {role, fio}: role '' — доступа нет; null — сервер не ответил (пускаем по пропуску).
 * Запрос подписан PORTAL_SECRET: sig = hex(HMAC-SHA256('логин|sb3|ts')), ts — unix-секунды.
 */
...
/**
 * ЗАПУСТИТЬ ОДИН РАЗ В РЕДАКТОРЕ (владелец) перед выкладкой входа через портал: Google спросит
 * разрешение «подключаться к внешним сервисам» — нажать «Разрешить». В журнале должен появиться
 * ответ сервера с "ok": true. (UrlFetchApp в СБ3 уже используется для Telegram, но адрес портала
 * лучше проверить.)
 */
