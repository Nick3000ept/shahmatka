...
- `renderAnalytics()` — строит таблицу по отфильтрованным строкам (те же фильтры FC/FO/FM/FL1/FL2/FW/FKP/FDATE, без FPLAN7/FPLAN30/FOVERDUE):
  - Считает по каждой работе: `week` (выполнено за 7д), `month` (за 30д), `total` (всего), `corpora{}` — разбивка по корпусам
  - Колонки таблицы: **Работа | 7 дн | 30 дн | Факт** (колонки «План» по baseDate и «Откл.» убраны 2026-07-15)
