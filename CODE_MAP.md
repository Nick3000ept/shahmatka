...
- `COL_KEYS[]` — порядок столбцов: `{corpus, place, lvl2, work, extra1, key, factNum}`.
  Сортировка: корпус → место → работа → подколонка. Места из `PLACE_ORDER` (Этаж, МОП,
  Квартира) — в заданном порядке, остальные (Лифт. холл, Лест. клетка …) — по самой ранней
  работе места. **Колонки одного места и одной работы обязаны идти подряд**: объединённые
  заголовки в шапке считаются по общему числу колонок группы, и при чередовании мест
  подписи «Место»/«Работа» съезжают относительно работ под ними (инцидент 2026-09-07 —
  «Лифт. холл» и «Лест. клетка» имели одинаковый приоритет 99 и перемешивались по factNum)
...
- `updateWorkInp()` — обновляет теги выбранных работ в поле ввода; блок тегов сворачиваемый (кнопка «▸ Работы: N», по умолчанию свёрнут)
- `toggleWorkTags()` — свернуть/развернуть блок тегов работ; состояние в localStorage `sb3_worktags_open` (перенесено из СБ5, 2026-09-02)
...
- `render()` — ОСНОВНАЯ ФУНКЦИЯ: фильтрует ROWS → строит HTML таблицы → вставляет в #board. В начале, если открыта «Сводная» (`#pivot-overlay.on`), перестраивает её (`buildPivotHtml`) — сводная реагирует на смену фильтров вживую
  - COL_KEYS строится за один O(n) проход по строкам (не вложенные циклы!), затем сортируется
    с сохранением непрерывности групп «место» и «работа» (см. COL_KEYS выше)
...
- `msCommit()` — применяет MS_DRAFT ко всем ячейкам SEL → MOD; **автодата** (2026-08-04): закрытие (статус `s-done` или 100%) ПЕРЕЗАПИСЫВАЕТ `dateEnd` сегодняшним числом; явно выбранная в msbar дата важнее; подстраховка для пустой даты осталась; **дата вне 2020…текущий+1 → сохранение отменяется с тостом (2026-09-10)**
...
- `collectAutoTasks()` / `floorsCompact(floors)` — сбор «плана на 7 дней» для формирования протокола (блок «Из графика» из панели убран 2026-08-04, `renderAutoTasks` удалена).
  `authors` — объект `{автор: дата последней его правки}` (`dateChg`), из него строится хвост «(поставил: … , изменено …)» (2026-09-07)
...
- `DATE_MIN_Y` / `dateMaxY()` — границы допустимого года в дате: 2020 … текущий год + 1 (2026-09-10)
- `dateInRange(d)` — проверка года в дате перед сохранением; используется в `savePanel()` и `msCommit()`
- `dateRangeMsg()` — текст тоста об ошибке диапазона дат
- `applyDateLimits()` — ставит `min`/`max` на поля `p-date` и `ms-date` (вызов в `window.onload`)
...
- `#work-tags-toggle` — кнопка сворачивания блока тегов работ («▸ Работы: N»)
