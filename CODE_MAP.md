...
- `exportPdf()` — таблица в `#print-area` → `window.print()` (как печать протокола); формат листа **А3**, ориентация автоматически: ≤12 колонок работ — книжная, больше — альбомная (`@page` во временном `<style id="exp-print-style">`, удаляется после печати — иначе сломает ориентацию печати протокола)
