...
- `buildExportTable()` — общая таблица для Excel и PDF: шапка Корпус→Место→Работа(→Доп.признак), строки — этажи, в ячейке `%` + дата (тек.→факт.→базовая, дд.мм), фон — цвет статуса (`EXP_ST_COLORS`; s-front — оранжевая заливка вместо рамки); возвращает `{html, cols}`
- `expFilterSummary()` — строка активных фильтров для шапки выгрузки; `expStamp()` — дата дд.мм.гггг; `expLegendHtml()` — легенда цветов статусов
...
- `exportPdf()` — таблица в `#print-area` → `window.print()` (как печать протокола); ориентация автоматически: ≤9 колонок работ — книжная, больше — альбомная (`@page` во временном `<style id="exp-print-style">`, удаляется после печати — иначе сломает ориентацию печати протокола)
