...
- Кнопка «📄 Сформировать протокол» → редактируемый предпросмотр (`#proto-overlay`, contenteditable-строки, ✕/+ строка) → «🖨 Печать / PDF»: сначала сохраняет текст в лист «Протоколы» (saveProtocol, автономер), затем window.print через `#print-area` (body.printing скрывает всё остальное).
