...
| POST | `sendProtocol` | Строит PDF из sections, шлёт каждому получателю отдельное письмо (MailApp, отправитель — аккаунт GAS), отметка в столбце E листа «Протоколы»; требует `pwd` |
...
- Структура протокола: первой группой «Общие поручения» (все открытые ручные поручения с подрядчиком и сроком), затем секции подрядчиков только с работами из графика.
- Рассылка: кнопка «📧 Отправить» в предпросмотре → окно получателей (подрядчики из протокола, галочки можно снять; «Альфа Строй» исключён константой `MAIL_EXCLUDE`; адреса из листа «Подрядчики» столбец E) → sendProtocol шлёт каждому отдельное письмо с PDF от kuzkin@acons.group. Решено НЕ через n8n (GAS-аккаунт и так отправитель, ноль настройки), n8n оставлен на будущее для Telegram.
